...
fn parse_time_input(input: &str) -> (String, Option<TimeUnit>) {
    ...
    for (unit_str, unit) in sorted_units {
        ...
        if trimmed.ends_with(unit_str) {
            ...
            if after_number > 0 {
                let numeric = if trimmed.as_bytes()[after_number - 1] == b' ' {
                    &trimmed[..after_number - 1]
                } else {
                    &trimmed[..after_number]
                };
...
pub fn time_input_widget(
    ...
    ui.horizontal(|ui| {
        ...
        if focused && !text_response.has_focus() {
            ui.ctx().memory_mut(|m| m.request_focus(text_response.id));
        }
