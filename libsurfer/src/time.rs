...
pub fn time_input_widget(
    ...
    ui.horizontal(|ui| {
        ...
        let text_response = ui.add(egui::TextEdit::singleline(&mut input).desired_width(100.0));
        ...
        if ui
            ...
            .clicked()
        {
            if let Some(time_stamp) = state.to_timescale_ticks(&waves.inner.metadata().timescale) {
                msgs.push(Message::GoToTime(Some(time_stamp), 0));
            }
        }
